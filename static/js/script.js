...
function updateUIForMode() {
  ...
  headerTitle.textContent = 'Pegasus-hacker';
  ...
  try {
    ...
    if (response.ok) {
      ...
      data.models.forEach(model => {
        ...
        option.value = model;
        option.textContent = model.replace("IHA089", "Sobri").replace("drana-infinity", "pegasus hacker").replace("Drana-Infinity", "Pegasus hacker");
...
function buildTerminalPrompt(command) {
  ...
  const line1 =
    ...
    `<span class="t-bright"><b>Sobri</b></span>` +
    `<span class="t-white">)</span>` +
    `<span class="t-white">-</span>` +
    `<span class="t-white">[</span>` +
    `<span class="t-bright"><b>Pegasus-hacker</b></span>` +
    `<span class="t-white">]</span>\n`;
